//! SHA-256 over byte slices and streams.
//!
//! Written out rather than pulled in: this crate runs in every org's CI before any
//! dependency is available, and a manifest generator that cannot start without a registry
//! is a manifest generator that will be skipped.
...
#[cfg(test)]
mod tests {
    use super::{hex, sha256_hex, Sha256};
    ...
    #[test]
    fn streams_in_arbitrary_chunks() {
        ...
        for chunk in [1usize, 7, 63, 64, 65, 128] {
            ...
            assert_eq!(hex(&hasher.finish()), once, "chunk size {chunk}");
        }
    }
}
